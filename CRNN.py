...
def KerasCrnn(img_height, n_labels, nc=1, nh=32):
    '''
...
#         nIn = nc if i == 0 else nm[i - 1]
        ...
        if( i == 0):
#             cnn.add( Conv2D( nOut, ks[i], strides=ss[i], input_shape=( img_height, None, 1 ), padding=padding, name='conv{0}'.format(i) ) )
            cnn.add(Conv2D(nOut, 3, strides=1, input_shape=(img_height, None, 1), padding=padding, name='conv{0}'.format(i)))
        else:
#             cnn.add( Conv2D( nOut, ks[i], strides=ss[i], padding=padding, name='conv{0}'.format(i) ) )
            cnn.add( Conv2D( nOut, ks[i], strides=1, padding=padding, name='conv{0}'.format(i) ) )
...
#     cnn.add( MaxPooling2D( pool_size=(2, 2), padding='valid', name='pooling{0}'.format(2) ) )  # 256x4x16
#     convRelu(4, True)
#     convRelu(5)
#     cnn.add( ZeroPadding2D( padding=(0,1) ))
# #     cnn.add( MaxPooling2D( pool_size=(2, 2), strides=(2, 1), padding='valid', name='pooling{0}'.format(3) ) )  # 512x2x16
#     cnn.add( MaxPooling2D( pool_size=(2, 2), padding='valid', name='pooling{0}'.format(3) ) )  # 512x2x16
...
#     cnn.add(Bidirectional( LSTM( nh , return_sequences=True, use_bias=True, recurrent_activation='sigmoid', )) )
#     cnn.add( TimeDistributed(Dense( n_labels ) ) )
